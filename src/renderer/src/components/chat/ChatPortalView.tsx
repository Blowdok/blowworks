  ...
  async function buildWikiContext(): Promise<string | null> {
    ...
    try {
      ...
      return [
        ...
        '',
        '**Format des références wiki dans TES réponses** (règle IMPORTANTE pour que les liens soient cliquables) :',
        "- Utilise systématiquement la syntaxe Obsidian : `[[nom-page]]` avec DEUX crochets (ex : `[[ethan-carter]]`, `[[promethee]]`).",
        "- N'écris JAMAIS `[nom-page]` avec un seul crochet — c'est de la syntaxe reference-style markdown invalide ici et l'utilisateur ne pourra pas cliquer dessus.",
        "- Pour mentionner un chemin explicite, tu peux aussi écrire `wiki/concepts/xxx.md` (le système transforme en lien automatiquement).",
        "- Ne cite JAMAIS `raw/…` (fichiers internes de maintenance).",
        "- Ne cite JAMAIS les `[slug]` comme liste de sources à la fin — glisse-les dans le corps du texte avec `[[slug]]` quand tu les mentionnes naturellement.",
        '',
