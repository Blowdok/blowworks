...
- **Menu IA dans le header** : bouton `✦ IA` à côté de `Chat`, ouvre un menu déroulant qui spawne directement une `BrowserShape` pré-chargée sur l'assistant choisi (ChatGPT, Claude, Gemini, Perplexity, Le Chat / Mistral, Grok, Copilot, DeepSeek, NotebookLM, HuggingChat). Chaque entrée affiche une pastille colorée à la couleur de la marque + une tagline courte décrivant la spécificité du service. Catalogue centralisé dans `src/shared/ai-services.ts` (ajouter un service = ajouter une entrée). Les sessions sont partagées entre tous les services via la partition `persist:browser` du webview — login persistant à travers les redémarrages, indépendamment isolé du renderer principal. Click extérieur / Échap referme le menu.
